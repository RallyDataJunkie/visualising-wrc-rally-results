...
```{r}
overall_wide_gap_top10 <- multi_overall_wide_gap_top10 %>%
                            gather(key ="Stage",
                          value ="Gap", stage_codes)

overall_wide_gap_top10 %>% head(3)
```

With the data in the appropriate form, we can create the sparkline, using a bar chart format:

```{r}
overall_wide_gap_long_top10 <- overall_wide_gap_top10 %>%
                                group_by(code) %>%
                                summarize(spk_ = spk_chr(-Gap,
                                                         type ="bar"))

# We need to create an htmlwidget form of the table
out = as.htmlwidget(formattable(overall_wide_gap_long_top10))
