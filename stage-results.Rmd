...
We can the merge this data into out original table, and filter out some of the less useful columns. We'll also limit ourselves to just the top 10 results:
...
We can add a caption to the table via the *caption* parameter:

```{r}
formattable( top10_stage_times, caption = caption)
```
...
For example, we can emphasise differences that exceed a specific amount: 
...
Alternatively, we can add a coloured bar that depicts the increasing gap time down the leaderboard:
...
formattable(top10_stage_times,
            list(Gap = color_bar2("#FA614B66") ))
```

### Heatmap Style Column Cell Backgrounds

As well as in cell bar charts, we can also use more of a heatmap style approach and colour the background down a column according to value.
...
The GAP between driver in first position,, $i=1$, and the driver in the $j$'th position is given in various abuses of the notation as:

$$GAP_{j}=t_{j,GAP}=t_{j,1,DIFF}=t_j-t_1$$
...
$$DIFF_{j,i}=t_{j,i,DIFF}=t_{j,i}=t_j-t_i: i<j, t_0=t_1$$

The GAP between a driver in position $j=P$ and the leader $i=1$ is then:

$$GAP_j=t_{j,GAP}=DIFF_{2,1}+DIFF_{3,2}+..+DIFF_{P,P-1}$$

We can write this more succinctly as:

$$GAP_{j}=t_{j,GAP}=0+\sum_{p=1}^{j}DIFF_{p,p-1}=\sum_{p=1}^{j}\left ( t_p-t_{p-1} \right ): j\ge1, t_0=t_1$$
...
To being with, we note that we can rebase a single column simply by finding the value associated with a particular driver in that column and subtracting that value from each row in the column.
...
To rebase more than one column, we can specify a list of columns we want to rebase and then process the response as a list before subtracting from each the corresponding columns in each row of the dataframe:
