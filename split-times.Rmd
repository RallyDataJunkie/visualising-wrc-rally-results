...
We can add this time to out ultimate times dataframe to provide an immediate point of reference betweem the actual best accumulated split times and the ultimate accumulated ideal split times:
...
There are various quick techiques we can use to help visualise the rebased split times and try to highlight significant patterns or peculiarities. For example, we can use coloured backgrounds to highlight each cell in a table, or sparklines to summarise each row.

### Context Sensistve Cell Colouring
...
Finally, let's see how the driver's actual split times varied gainst the best recorded split time at each split:
...
A quick way of summarising rebased times in a more space efficient way is to us a sparkline. As we have seen previously, these can be used added as an extra column alongside a row of data, oer could be used to as a quick visual indicative summary of a row of values.
