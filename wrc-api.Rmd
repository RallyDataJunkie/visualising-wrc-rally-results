...
We can get a list of stage IDs from the `stageId` column (`stages$stageId`); or we can create a lookup from code to stage ID:
...
At the end of each stage, there are actually two different sorts of results data availbe: data relating to the result of the stage itself, and data relating to how the stage result affected the overall rally position.
...
multi_overall_results = get_multi_overall(stagelist)
...
multi_stage_times = get_multi_stage_times(stagelist)
...
Now create a function to get the driver splits in a wide format using the order split point codes as the widened columns:
